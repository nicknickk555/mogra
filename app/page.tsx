      ...
      <section className={`portal-deck panel portal-stage-${active}`}>
        <div className="holo-world" aria-label={`${stage.nav} interactive visual`}>
          ...
          <div className="chart-portal">
            <div className="chart-portal-equation">C(s*) = C<sub>crit</sub></div>
        ...
        <div className="portal-story" key={active}>
          ...
          <div className="portal-formula"><strong>{stage.formula}</strong></div>
          ...
          <div className="portal-actions">
            ...
            {active > 0 && <button type="button" className="portal-back" onClick={() => setActive(active - 1)}>Previous</button>}
          </div>
      ...
      <section className="portal-collection" aria-label="Digital specimen collection">
        {stages.map((item, index) => (
          <button key={item.nav} type="button" onClick={() => setActive(index)} className={active === index ? "active" : ""}>
            ...
            <div><strong>{item.nav}</strong><p>{index === 0 ? "Boundary field" : index === 1 ? "Biological signal" : "Moisture model"}</p></div>
          </button>
        ))}
        <Link href="/explorer" className="collection-gateway"><strong>Enter Screen 2</strong></Link>
      </section>
    </main>
  );
}
